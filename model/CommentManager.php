...
class CommentManager extends Manager
{
    ...
    {
        $db = $this->dbConnect();
        $req= $db->prepare('SELECT commentId, author, comment, stage, DATE_FORMAT(commentDate, \'%d/%m/%Y à %Hh%i\') AS commentDate_fr FROM comments 
        WHERE postId = ? AND  visibility = 1  ORDER BY commentDate DESC');
